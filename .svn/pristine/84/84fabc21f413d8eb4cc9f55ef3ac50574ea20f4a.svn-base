...
class MysqlModel extends BpfModel
{
  ...
  private function _getKeyValues($set)
  {
    ...
    private function _getKeyValues2($set)
  {
    $keys = array();
    $values = array();
    if (!is_array(reset($set))) {
    ...
    foreach ($set as $key => $value) {
      ...
      foreach ($value as $key2 => $value2) {
        if ($i == 0) {
          $keys[] = '`' . $this->escape($key2) . '`';
        }
        $field = true;
        if (is_array($value2) && isset($value2['value'])) {
          if (isset($value2['escape']) && false === $value2['escape']) {
            $field = false;
          }
          $value3 = $value2['value'];
        }
        if ($field) {
          $value3 = '"' . $this->escape($value2) . '"';
        }
        $row[] = $value3;       
      }
      $values[] = '(' . implode(', ', $row) . ')';
...
//批量插入 fooker作品
  public function inserts($table, $set, $ignore = false )
  {
    if (!$set || !is_array($set)) {
      return new BpfExecuteResult(null, false);
    }
    $keyValues = $this->_getKeyValues2($set);
    $sql = 'INSERT ' . ($ignore ? 'IGNORE ' : '') . 'INTO `' . $this->escape($table) . '` (' .
        implode(', ', $keyValues['keys']) . ') VALUES ' . implode(', ', $keyValues['values']);
    return $this->query($sql);
  }
